...
export default function SaasAdminPage() {
  ...
  const fetchDistribuidoras = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/superadmin/distribuidoras`, {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-[#4a6c6f] p-6 sm:p-8 rounded-2xl text-white shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <span className="inline-flex items-center gap-1 bg-white dark:bg-slate-900/20 px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full mb-3">
        ...
        <button 
          ...
          className="shrink-0 flex items-center justify-center gap-2 px-5 py-3 bg-white dark:bg-slate-900 text-[#4a6c6f] hover:bg-slate-50 dark:bg-slate-950 rounded-xl font-bold transition-all shadow-md"
        >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-slate-500 dark:text-slate-400 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">No hay distribuidoras activas</h3>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {distribuidoras.map((dist) => (
                  <tr key={dist.id} className="hover:bg-slate-50 dark:bg-slate-950/50 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-bold text-slate-900 dark:text-white">{dist.nombre}</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="font-medium text-slate-900 dark:text-white">{dist.usuario.nombre}</p>
                      <p className="text-sm text-slate-500 dark:text-slate-400">{dist.usuario.correo}</p>
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-6 md:p-8 max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Nueva Distribuidora</h2>
            <p className="text-slate-500 dark:text-slate-400 mb-6">Crea los accesos y el perfil de tu nuevo cliente.</p>
            ...
            <form onSubmit={handleCreate} className="space-y-5">
              ...
              <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-800 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Nombre Comercial de la Tienda</label>
                  <input
                    type="text"
                    required
                    value={nombreTienda}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Teléfono de WhatsApp (Recibe Pedidos)</label>
                  <input
                    type="tel"
                    required
                    value={telefono}
              ...
              <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-800 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Nombre del Dueño/Representante</label>
                  <input
                    type="text"
                    required
                    value={nombreDueno}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Correo (Para Iniciar Sesión)</label>
                  <input
                    type="email"
                    required
                    value={correo}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Contraseña Temporal</label>
                  <input
                    type="text"
                    required
                    value={contrasena}
              ...
              <div className="pt-4 flex justify-end gap-3">
                <button
                  ...
                  className="px-5 py-2.5 text-slate-600 dark:text-slate-300 font-medium hover:bg-slate-100 dark:bg-slate-800 rounded-xl transition-colors"
                >
                  Cancelar
                </button>
                <button
