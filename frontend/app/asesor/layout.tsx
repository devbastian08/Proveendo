      ...
      <header className="bg-white dark:bg-slate-900 text-[#4a6c6f] shadow-sm border-b border-slate-200 dark:border-slate-700 sticky top-0 z-40">
        ...
        <div className="absolute right-4 top-1/2 -translate-y-1/2 z-10 hidden sm:flex items-center gap-3">
          {isClient && user?.puedeAlistar && (
            <button 
              onClick={goToBodega}
              className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-[#4a6c6f] hover:bg-slate-50 dark:bg-slate-950 rounded-full transition-colors border border-[#4a6c6f]/20 hover:border-[#4a6c6f]/50 shadow-sm"
              title="Ir a Bodega"
            >
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2 z-10 flex sm:hidden items-center gap-1">
          {isClient && user?.puedeAlistar && (
            <button 
              onClick={goToBodega}
              className="flex items-center justify-center w-10 h-10 text-[#4a6c6f] hover:bg-slate-50 dark:bg-slate-950 rounded-full transition-colors"
              title="Ir a Bodega"
            >
